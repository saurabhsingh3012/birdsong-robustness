...
curves are in the README's real-audio results section (`docs/real_eval_results.json`). That
closes Milestones 1 and 2 below. What remains is making the degradations *realistic* (not just
calibrated) and scaling the evaluation past six easy species, the remaining gaps, in order.
...
- [ ] Report the named composite conditions (`COMPOSITE_CONDITIONS`) in full: distant
